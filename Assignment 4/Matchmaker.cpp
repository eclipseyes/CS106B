...
//map.firstKey()   a key out of a Map
//set.first()   a key out of a Set



bool hasPerfectMatching(const Map<string, Set<string>>& possibleLinks, Set<Pair>& matching) {
    // base case: if there's a node with no possible links, return false
    ...
    // base case: if all nodes are matched, return true
    if (possibleLinks.isEmpty()) {
        return true;
    }

    // recursive case: pick the first node and attempt all possible pairings
    string choosed_key = possibleLinks.firstKey();
    for (const string& choosed_ref : possibleLinks[choosed_key]) {
        Map<string, Set<string>> newPossibleLinks = possibleLinks;

        // remove the chosen pair from newPossibleLinks
        newPossibleLinks.remove(choosed_key);
        newPossibleLinks.remove(choosed_ref);

        // remove all links connected to choosed_key and choosed_ref
        for (const auto& key : newPossibleLinks) {
            newPossibleLinks[key].remove(choosed_key);
            newPossibleLinks[key].remove(choosed_ref);
        }

        // add this pair to the matching and recurse
        matching.add({choosed_key, choosed_ref});
        if (hasPerfectMatching(newPossibleLinks, matching)) {
            return true;
        }
        matching.remove({choosed_key, choosed_ref}); // backtrack
    }

    // if no pairing works, return false
    return false;
}


// possibleLinks["A"]["E"] gives the weight of the potential link between A and E
// 声明 maximumWeightMatchingHelper 函数


int weightSum(const Map<string, Map<string, int>>&Links, Set<Pair> pairs){
    int sum=0;
    for(Pair pair: pairs){
...
Set<Pair> betterMatching(const Map<string, Map<string, int>>&Link1, const Map<string, Map<string, int>>&Link2, Set<Pair> pair1, Set<Pair> pair2){
    return (weightSum(Link1,pair1)>weightSum(Link2,pair2)?pair1:pair2);
}

Set<Pair> maximumWeightMatchingHelper(const Map<string, Map<string, int>>& possibleLinks_1, const Set<Pair> chosen_Pair) {
    //base case: can not match or already match all:  return this pair

    auto possibleLinks=possibleLinks_1;
    for (const auto& key : possibleLinks_1) {
        if (possibleLinks_1[key].isEmpty()) {
    ...
    if (possibleLinks.isEmpty()) {
        ...
        Set<Pair> better_pair;

        string choosed_key = possibleLinks.firstKey();   //recursive case: choose a pair

            string choosed_ref=possibleLinks[choosed_key].firstKey();

            //with this pair
            auto newPossibleLinks_1= possibleLinks;

            // remove the chosen pair from newPossibleLinks
            newPossibleLinks_1.remove(choosed_key);
            newPossibleLinks_1.remove(choosed_ref);

            // remove all links connected to choosed_key and choosed_ref
            for (const auto& key : newPossibleLinks_1) {
                newPossibleLinks_1[key].remove(choosed_key);
                newPossibleLinks_1[key].remove(choosed_ref);
            }

            //without this pair
            auto newPossibleLinks_2= possibleLinks;

            newPossibleLinks_2[choosed_key].remove(choosed_ref);
            newPossibleLinks_2[choosed_ref].remove(choosed_key);

        //with this pair or without this pair, return the better one
            Set<Pair> more_pair=chosen_Pair;
            Pair another_pair(choosed_key,choosed_ref);
            more_pair.add(another_pair);
            Set<Pair> with=maximumWeightMatchingHelper(newPossibleLinks_1,more_pair);
            Set<Pair> without=maximumWeightMatchingHelper(newPossibleLinks_2,chosen_Pair);
            better_pair= betterMatching(possibleLinks,possibleLinks,with,without);
...
Set<Pair> maximumWeightMatching(const Map<string, Map<string, int>>& possibleLinks) {    
    auto positiveLinks=possibleLinks;
    //delete all the non-positive weight edge;
    for(string key: possibleLinks){
        for(string ref: possibleLinks[key]){
            if(possibleLinks[key][ref]<=0){
                positiveLinks[key].remove(ref);
            }
        }
    }
    Set<Pair> pair;
    return maximumWeightMatchingHelper(positiveLinks,pair);
}
...
STUDENT_TEST("hasPerfectMatching works on a pentagon of people.") {
    /* Here's the world:
     *
     *               A --- B
     *                \   /
...
STUDENT_TEST("maximumWeightMatching: Works on a i-do-not-know.") {
    ...
    EXPECT_EQUAL(maximumWeightMatching(links),  {{"B", "F"}, {"A", "E"},{"D", "C"}});
}
