...
/************EXTENSION FUNCTION:pick the uncovered city with the fewest neighbors************/

string leastNeighborCity(Set<string>& notReady,const Map<string, Set<string>>& roadNetwork){
    string targetCity=notReady.first();
    int least_neighbor=roadNetwork[targetCity].size();
    int neighbor;
    for(string city:notReady){
        neighbor=roadNetwork[city].size();
        if(neighbor<least_neighbor){
...
Set<string> adjacentCity(const Map<string, Set<string>>& roadNetwork,const string choosed_city){
    Set<string> cities;
    cities.add(choosed_city);
    for(string city:roadNetwork[choosed_city]){
        cities.add(city);
    }
    return cities;
}
...
bool canBeMadeDisasterReadyRec(const Map<string, Set<string>>& roadNetwork,
                            int numCities,
                            Set<string>& supplyLocations, Set<string>& notReady) {
    //base case:
    //if every one city has been covered, return true
    if(notReady.isEmpty()){
        return true;
    }
    if(numCities==0){  //there are still cities not ready, but left with no supplies
        return false;
    }
    //recursive case :
    //choose one city that has not been covered
    string city= leastNeighborCity(notReady,roadNetwork);
        //try to cover this city, explore to see the rest can be made ready or not
    supplyLocations.add(city);
    Set<string> cityInPower=notReady * adjacentCity(roadNetwork,city);
                              notReady-=cityInPower;
    bool coverThisCity=canBeMadeDisasterReadyRec(roadNetwork,numCities-1,supplyLocations,notReady);
    if(coverThisCity){   // if covering this city can work , return true
        return true;
    }else{               //if covering this city can not work:
        supplyLocations.remove(city);  //uncover this city
        notReady+=cityInPower;
        for(string adjacent: roadNetwork[city]){  //make iterations to try to cover adjacent cities
            supplyLocations.add(adjacent);
            Set<string> cityAdjacentInPower=notReady * adjacentCity(roadNetwork,adjacent);
            notReady-=cityAdjacentInPower;
            if(canBeMadeDisasterReadyRec(roadNetwork,numCities-1,supplyLocations,notReady)){
                return true;   //if any can work return true
            }
            supplyLocations.remove(adjacent);
            notReady+=cityAdjacentInPower;
        }
...
bool canBeMadeDisasterReady(const Map<string, Set<string>>& roadNetwork,
    ...
    if(numCities<0){
        error("you can not choose cities less than zero.");
    }
    ...
    return canBeMadeDisasterReadyRec(roadNetwork,numCities,supplyLocations,notReady);
}
...
STUDENT_TEST("test a network designed by me") {
