...
Map<string, int> apportion(const Map<string, int>& populations, int numSeats) {
    if(populations.size()>numSeats){
        error("the seats cannot be more than the number of states");
    }
    Map<string,int> seats;
    HeapPQueue de_population;
    for(string state: populations){
        seats[state]=1;
        de_population.enqueue({state,-1.0*populations[state]/sqrt(2)});
    }
    numSeats-=populations.size();
    while(numSeats>0){
        string state=de_population.dequeue().name;
        seats[state]++;
        double adjusted_weight=-1.0*populations[state]/sqrt(seats[state]*(seats[state]+1));
        de_population.enqueue({state,adjusted_weight});
        numSeats--;
    }
    return seats;
}
...
STUDENT_TEST("Try a simple model") {
    ...
    EXPECT_EQUAL(apportion(populations, 5), seats);
}
