...
void HeapPQueue::enqueue(const DataPoint& data) {     // 0    1 2 3 4 5 6 7 8
    if(logicalSize+1==allocatedSize){                          //11 11 1 1 1 1 1
        grow();
    }
    int index=logicalSize+1;
    elems[index]=data;
    while(index>1&&elems[index].weight<elems[index/2].weight){
...
DataPoint HeapPQueue::dequeue() {
    ...
    int index=1;
    while((index*2+1<logicalSize &&(elems[index].weight>elems[index*2].weight||elems[index].weight>elems[index*2+1].weight))||
           (index*2<logicalSize && elems[index].weight>elems[index*2].weight))
    {
        if(index*2+1<logicalSize){
            double father_weight= elems[index].weight;
            double child_one_weight= elems[index*2].weight;
            double child_two_weight=elems[index*2+1].weight;
            if(father_weight>child_one_weight&&child_one_weight<child_two_weight){
                    swap(elems[index],elems[index*2]);
                    index=index*2;

            }else{
                    swap(elems[index],elems[index*2+1]);
                    index=index*2+1;
                }
            }
        else{
...
void HeapPQueue::grow(){
    allocatedSize*=2;
    DataPoint* newElem=new DataPoint[allocatedSize];
...
STUDENT_TEST("Handles large sequence of interleaved enqueues and dequeues correctly.") {
    HeapPQueue pq;
    const int numOperations = 50000;

    // Step 1: Enqueue elements with weights from 0 to numOperations - 1
    for (int i = 0; i < numOperations; i++) {
        pq.enqueue({ "elem" + to_string(i), double(i) });
    }

    EXPECT_EQUAL(pq.size(), numOperations);

    // Step 2: Dequeue half of them
    for (int i = 0; i < numOperations / 2; i++) {
        EXPECT_EQUAL(pq.dequeue().weight, i);
    }

    EXPECT_EQUAL(pq.size(), numOperations / 2);

    // Step 3: Interleave enqueue and dequeue operations
    for (int i = numOperations; i < numOperations + 1000; i++) {
        pq.enqueue({ "elem" + to_string(i), double(i) });
        EXPECT_EQUAL(pq.dequeue().weight, i - (numOperations / 2));
    }

    // At this point, the heap should still be properly structured, with the smallest elements on top
    while (!pq.isEmpty()) {
        DataPoint current = pq.dequeue();
        if (!pq.isEmpty()) {
            DataPoint next = pq.peek();
            EXPECT(current.weight <= next.weight); // Ensure heap property is maintained
        }
    }
