...
Grid<bool> floodedRegionsIn(const Grid<double>& terrain,const Vector<GridLocation>& sources,double height) {
    Grid<bool> flooded_terrain(terrain.numRows(),terrain.numCols(),false);
    Queue<GridLocation> flooded_spot;

    for(GridLocation source :sources){
        if(terrain[source]<=height){
            flooded_terrain[source]=true;
            flooded_spot.enqueue(source);
        }
    }
        GridLocation spot;

    while(!flooded_spot.isEmpty()){
        spot=flooded_spot.dequeue();
        //find the adjacent spot
        Vector<GridLocation> adjacent_spot;

        if(terrain.inBounds(spot.row,spot.col-1)){
            adjacent_spot.add({spot.row,spot.col-1});
        }
        if(terrain.inBounds(spot.row,spot.col+1)){
            adjacent_spot.add({spot.row,spot.col+1});
        }
        if(terrain.inBounds(spot.row-1,spot.col)){
            adjacent_spot.add({spot.row-1,spot.col});
        }
        if(terrain.inBounds(spot.row+1,spot.col)){
            adjacent_spot.add({spot.row+1,spot.col});
        }

        //each square adjacent to the spot
        for(GridLocation adjacent :adjacent_spot){
            if(flooded_terrain[adjacent]==false && terrain[adjacent]<=height){
                flooded_terrain[adjacent]=true;
                flooded_spot.enqueue(adjacent);
            }
        }
    }
    return flooded_terrain;
}
...
STUDENT_TEST("try another world designed by me.") {
    Grid<double> world = {
        { 4,4,0,2,5,2,-2,2 },
        { 5,0,-1,6,5,7,3,1},
        { -1,6,0,-2,3,0,2,3},
