...
Map<string, double> kGramsIn(const string& str, int kGramLength) {
    ...
    Map<string, double> kGramContainer;
    if(str.length() < kGramLength){
        return kGramContainer;
    }
    for(int i=0; i+kGramLength<=str.length();i++){
        kGramContainer[str.substr(i,kGramLength)]++;
    }
    return kGramContainer;
...
Map<string, double> normalize(const Map<string, double>& input) {
    Map<string, double> normalizeMap;
    double square_sum;
    square_sum=0;
    for(string kGram: input ){
        square_sum+=(input[kGram]*input[kGram]);
    }
    if(square_sum==0){
        error("empty map");
    }
    for(string kGram: input ){
        normalizeMap[kGram]=input[kGram]/sqrt(square_sum);
    }
    return normalizeMap;
...
Map<string, double> topKGramsIn(const Map<string, double>& source, int numToKeep) {
    PriorityQueue<string> frequency;
    Map<string, double> topKGrams=source;
    if(numToKeep<0){
        error("you cannot leave negative numbers of map");
    }
    for(string kGram: source){
        frequency.enqueue(kGram,source[kGram]);
    }
    while(numToKeep < topKGrams.size()){
        topKGrams.remove(frequency.dequeue());
    }
    return topKGrams;
}
...
double cosineSimilarityOf(const Map<string, double>& lhs, const Map<string, double>& rhs) {
    double result=0;
    for(string kGrams: lhs){
        if(rhs.containsKey(kGrams)){
            result+=lhs[kGrams]*rhs[kGrams];
        }
    }
    return result;
}
...
string guessLanguageOf(const Map<string, double>& textProfile,
    ...
    PriorityQueue<string> language_difference;
    double cosine_probability;
    double max_probability=-1.0;
    string bestLanguage="";
    for(const Corpus& reference: corpora){
        cosine_probability=cosineSimilarityOf(textProfile,reference.profile);
        if(cosine_probability>max_probability){
            max_probability=cosine_probability;
            bestLanguage=reference.name;
        }
    }
    return bestLanguage;
